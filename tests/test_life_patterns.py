...
def test_build_life_patterns_orchestration(monkeypatch):
    ...
    monkeypatch.setattr(life_patterns, "generate_embeddings", fake_generate_embeddings)
    monkeypatch.setattr(life_patterns, "choose_cluster_count", lambda embeddings : 3)
    fake_clusterer = object()
    monkeypatch.setattr(life_patterns, "fit_note_clusters", lambda embeddings, total_clusters:(fake_clusterer, fake_labels))
    monkeypatch.setattr(
        life_patterns,
    ...
    result = life_patterns.build_life_patterns(df)
    assert len(result) == 2
